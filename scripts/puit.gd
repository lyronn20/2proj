...
var last_water_point := Vector2.ZERO  
...
func get_point_eau() -> Vector2:
	...
	var voisins = []
	var plus_proches = Vector2i.ZERO
	voisins = [Vector2i(1, 0), Vector2i(-1, 0), Vector2i(0, 1), Vector2i(0, -1)]
	plus_proches = Vector2i(-9999, -9999)
	var found := false
	for v in voisins:
		var voisin = cell + v
	...
	if found:
		return game.route_tilemap.map_to_local(plus_proches) + game.route_tilemap.global_position
	else:
		return global_position
